...
int main() {
  ...
  result = calcCircle(CirclePool[1], CirclePool[2], CirclePool[3]);
  printf("%.6lf %.6lf %.6lf %.12lf\n", result.center.x, result.center.y, result.center.z, result.radius);
  printf("%.6lf %.6lf\n", CirclePool[1].radius, distance(CirclePool[1], result));
  printf("%.6lf %.6lf\n", CirclePool[2].radius, distance(CirclePool[2], result));
  printf("%.6lf %.6lf\n", CirclePool[3].radius, distance(CirclePool[3], result));
  printf("%d\n", int(result.radius/M_PI_2 * 100000));
  //putbuf();
  return 0;
}
